...
//Class AvgWidgetTempPart2 Defined
public class AvgWidgetTempPart2 extends Configured implements Tool {
  ...
  public int run(String [] args) throws Exception {
    ...
    job.setJarByClass(AvgWidgetTempPart2.class);

    job.setMapperClass(MaxWidgetMapper.class);
    job.setReducerClass(MaxWidgetReducer.class);

    FileInputFormat.addInputPath(job, new Path("week-06-sqoop"));
    FileOutputFormat.setOutputPath(job, new Path("avgTempSqp"));
  ...
  public static void main(String [] args) throws Exception {
    int ret = ToolRunner.run(new AvgWidgetTempPart2(), args);
    System.exit(ret);
  }
}
